...
class Chef::ResourceDefinitionList::MongoDB
  def self.create_connection(node, host, port, opts = {})
    require 'rubygems'
    require 'mongo'

    connection = Mongo::MongoClient.new(host, port, opts)
  ...
  def self.create_replset_connection(node, hosts, opts = {})
    require 'rubygems'
    require 'mongo'

    connection = Mongo::MongoReplicaSetClient.new(hosts, opts)
    admin = connection.db('admin')
  ...
  def self.configure_replicaset(node, name, members)
    ...
    begin
      ...
      rescue_connection_failure do
        connection = self.create_connection(node, 'localhost', node['mongodb']['config']['port'], :op_timeout => 5, :slave_ok => true)
    ...
    if result.fetch('ok', nil) == 1
      ...
      begin
        connection = self.create_connection(node, server, port, :op_timeout => 5, :slave_ok => true)
      rescue
      ...
      if config['_id'] == name && config['members'] == rs_members
        ...
        rs_connection = nil
        rescue_connection_failure do
          rs_connection = self.create_replset_connection(node, old_members)
  ...
  def self.configure_shards(node, shard_nodes)
    ...
    begin
      connection = self.create_connection(node, 'localhost', node['mongodb']['config']['port'], :op_timeout => 5)
    rescue => e
      Chef::Log.warn("Could not connect to database: 'localhost:#{node['mongodb']['config']['port']}', reason #{e}")
      return
    end
